...
function unlockMove() {
  ...
  fabric.Object.prototype.hoverCursor = false;
...
function lockMove() {
  ...
  fabric.Object.prototype.hoverCursor = true;
...
function boost(v) {
  ...
  function boostLine(line) {
    ...
    let [ctPrime, xPrime] = lorentz(ct, x);
    line.set({
      x1: ctPrime + canvas.width / 2,
      y1: canvas.height / 2 - xPrime,
    });
    ...
    line.set({
      x2: ctPrime + canvas.width / 2,
      y2: canvas.height / 2 - xPrime,
    });
  }
...
function setUpToolbar() {
  ...
  boostTool.addEventListener("click", () => {
    let velocity =
      c * parseFloat(window.prompt("Boost to velocity (multiple of c): "));
    boost(velocity);
  });
